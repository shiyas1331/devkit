...
# /devkit:cover:hooks — hook batch (all 3 sub-classifications)

Equivalent to `/devkit:cover <path> --batch hooks`. Covers `hook-pure`, `hook-redux`, `hook-bottomsheet`. Skips the picker.
...
## Phase 1 — Discover (silent, filtered to hooks)

Spawn `codebase-locator` to enumerate untested `.ts/.tsx` files under `<PLATFORM_ROOT>` matching `use*.{ts,tsx}` that export the function. Sub-classify each as:
...
## Phase 2 — Load templates + conventions ONCE

For each sub-classification present in the batch:
1. Read `<plugin-root>/platforms/<PLATFORM>/templates/hook-pure.template.md` → cached as `TEMPLATE_PURE`
2. Read `<plugin-root>/platforms/<PLATFORM>/templates/hook-redux.template.md` → cached as `TEMPLATE_REDUX`
3. Read `<plugin-root>/platforms/<PLATFORM>/templates/hook-bottomsheet.template.md` → cached as `TEMPLATE_BS`

Read `<plugin-root>/platforms/<PLATFORM>/conventions.md` → `CONVENTIONS_CONTENT`

## Phase 3 — Spawn agents (parallel, pool of 5)

For each untested hook file, look up the cached template matching its sub-classification, then:

```
PLATFORM=<PLATFORM>
SOURCE_FILE=<absolute path>
CLASSIFICATION=hook-pure | hook-redux | hook-bottomsheet
PACKAGE_ROOT=<PLATFORM_ROOT>
EXISTING_FIXTURES=<comma-separated list>

TEMPLATE:
<paste the matching template verbatim>
...
Done. To commit:
  ...
  git commit -m "test(<TICKET>): cover hooks batch"
  git push
```
