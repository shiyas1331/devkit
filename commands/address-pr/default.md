---
description: Address PR review feedback — walk through each comment with confirmation, commit fixes, post replies. Default mode when invoked with a bare PR identifier.
argument-hint: <PR url, PR number, or branch name>
model: opus
---

# /devkit:address-pr:default — walk-through mode (canonical pipeline)

Equivalent to `/devkit:address-pr <PR>` (bare PR identifier, no flag). This file is the **canonical pipeline** — other `address-pr/*` sub-commands (`dry-run`, `ignore-bots`, `auto-resolve`) reference it and modify behavior at specific phases.
...
- Bot comments if `--ignore-bots` flag was layered on (detect by `user.type=="Bot"` or login patterns: `coderabbit`, `dependabot`, `danger`, `*-bot`)
...
**Note: in `--dry-run` mode (see `dry-run.md`), the pipeline halts here — no further phases run.**
...
4. For comments where a fix was applied:
   - If `--auto-resolve` flag was layered on (see `auto-resolve.md`), resolve via GraphQL without prompting.
   - Otherwise, ask per comment.
   ```
...
- DO NOT apply or post without explicit confirmation.
